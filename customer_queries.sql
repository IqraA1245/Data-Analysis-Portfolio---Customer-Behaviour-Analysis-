   ...
   SELECT TOP 20 *
   FROM customer_behaviour..clean_customer_behaviour$
...
from customer_behaviour..clean_customer_behaviour$
where discount_applied = 'Yes' and purchase_amount_usd >= (select AVG(purchase_amount_usd) from clean_customer_behaviour$)
...
ROUND(AVG(purchase_amount_usd),2) as 'Average purchase amount'
from customer_behaviour..clean_customer_behaviour$
